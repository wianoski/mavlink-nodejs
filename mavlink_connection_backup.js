...
var timeout_interval_heartbeat = 1000;
...
function open_serial() {
  ...
  parser = new mavlink(system_ID, 1);
  ...
  parser.on('ready', function () {
    ...
    parser.on('message', function (message, fields) {
      if (message.id == 26) {
    ...
    parser.on('RADIO', function (message, fields) {
    });
    parser.on('RADIO_STATUS', function (message, fields) {
    });


    // HEARTBEAT message
    parser.on('HEARTBEAT', function (message, fields) {
      gcsDate = new Date();
    ...
    parser.on('SYS_STATUS', function (message, fields) {
    ...
    // ATTITUDE message
    parser.on('ATTITUDE', function (message, fields) {
    ...
    // ATTITUDE message
    parser.on('LOCAL_POSITION', function (message, fields) {
      emitter.emit('status_text', fields);
    });

    // GLOBAL_POSITION_INT message
    parser.on('GLOBAL_POSITION_INT', function (message, fields) {
    ...
    // GPS_RAW_INT message
    parser.on('GPS_RAW_INT', function (message, fields) {
      gps_raw_int = fields;
      emitter.emit('gps_raw_int', gps_raw_int);
    });

    // RC_CHANNELS_RAW message
    parser.on('RC_CHANNELS_RAW', function (message, fields) {
    });

    // RC_CHANNELS_RAW message
    parser.on('RC_CHANNELS_SCALED', function (message, fields) {
      //emitter.emit('rc_channels', fields);
    });

    // SERVO_OUTPUT_RAW message
    parser.on('SERVO_OUTPUT_RAW', function (message, fields) {
      emitter.emit('servo_output', fields);
    });

    // STATUS_TEXT message
    parser.on('STATUSTEXT', function (message, fields) {
    ...
    //  message
    parser.on('RAW_IMU', function (message, fields) {
      emitter.emit('raw_imu', fields);
    });

    //  message
    parser.on('SCALED_IMU2', function (message, fields) {
      emitter.emit('raw_imu2', fields);
    });

    // NAV_CONTROLLER_OUTPUT message
    parser.on('NAV_CONTROLLER_OUTPUT', function (message, fields) {
    });

    // MEMINFO message
    parser.on('MEMINFO', function (message, fields) {
    });

    // MISSION_CURRENT message
    parser.on('MISSION_CURRENT', function (message, fields) {
    ...
    // VFR_HUD message
    parser.on('VFR_HUD', function (message, fields) {
    });

    // AHRS message
    parser.on('AHRS', function (message, fields) {
    });

    // SYSTEM_TIME message
    parser.on('SYSTEM_TIME', function (message, fields) {
    });

    // HWSTATUS message
    parser.on('HWSTATUS', function (message, fields) {
    });

    // VIBRATION message
    parser.on('VIBRATION', function (message, fields) {
    });

    // COMMAND_ACK message
    parser.on('COMMAND_ACK', function (message, fields) {
      emitter.emit('command_ack', fields);
    });

    // AUTOPILOT_VERSION message
    parser.on('AUTOPILOT_VERSION', function (message, fields) {
    });

    // MISSION_ITEM message
    parser.on('MISSION_ITEM', function (message, fields) {
    ...
    parser.on('MISSION_COUNT', function (message, fields) {
      mission_len = fields.count;
      mission_get_all();
    });

    // MISSION_ACK message
    parser.on('MISSION_ACK', function (message, fields) {
    ...
    // MISSION_ACK message
    parser.on('MISSION_REQUEST', function (message, fields) {
      emitter.emit('status_text', fields);
      var mission_index = fields.seq;
      var mission_procedure = {
        index: mission_index + 1,
    ...
    //  message
    parser.on('PARAM_VALUE', function (message, fields) {
      emitter.emit('status_text', fields);
    });
...
function send_heartbeat() {
  ...
  if (heartbeat_enabled && parser != undefined) {
    parser.createMessage("HEARTBEAT", {
      'custom_mode': 0,
...
function set_datastream_rate(id, rate) {
  parser.createMessage("REQUEST_DATA_STREAM", {
...
function send_version() {
  parser.createMessage("AUTOPILOT_VERSION_REQUEST", {
    'target_system': system_ID,
    'target_component': 1
  },
    function (message) {
...
function send_command_long(param1, param2, param3, param4, param5, param6, param7, command, confirmation) {
  parser.createMessage("COMMAND_LONG", {
    'param1': param1,
    'param2': param2,
    'param3': param3,
    'param4': param4,
...
function mission_request_individual(seq) {
  ...
  parser.createMessage("MISSION_REQUEST", {
    'seq': seq,
    'target_system': system_ID,
    'target_component': 1
  },
...
function mission_request_list() {
  ...
  mission_len = -1;
  parser.createMessage("MISSION_REQUEST_LIST", {
    'target_system': system_ID,
    'target_component': 1
  },
    function (message) {
...
function mission_clear_all() {
  parser.createMessage("MISSION_CLEAR_ALL", {
    'target_system': system_ID,
    'target_component': 1
  },
    function (message) {
...
function mission_set_current(seq) {
  parser.createMessage("MISSION_SET_CURRENT", {
    'seq': seq,
    'target_system': system_ID,
    'target_component': 1
  },
...
function mission_write_waypoint(fields) {
  parser.createMessage("MISSION_ITEM", {
...
function init_mission_trans() {
  ...
  parser.createMessage("MISSION_COUNT", {
    'count': mission_len,
    'target_system': system_ID,
    'target_component': 1
  },
...
function send_rc_override(gcs_joystick) {
  ...
  parser.createMessage("RC_CHANNELS_OVERRIDE", {
...
function send_offboard_control(joystick) {
  if (connection_lost) return;
  parser.createMessage("OFFBOARD_CONTROL", {
...
function param_request_single(param_id) {
  parser.createMessage("PARAM_REQUEST_READ", {
    'param_id': param_id,
    'param_index': -1,
    'target_system': system_ID,
    'target_component': 1
...
function param_set_value(param_id, param_value, param_type) {
  parser.createMessage("PARAM_SET", {
    'param_id': param_id,
...
function guide_set_pos(x, y, z) {
  parser.createMessage("SET_POSITION_TARGET_LOCAL_NED", {
    'time_boot_ms': 0,
    'target_system': system_ID,
    'target_component': 1,
    'coordinate_frame': 9,
...
module.exports = {
  ...
  mission_chaging_speed: mission_chaging_speed,
